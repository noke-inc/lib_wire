...
func Generate(ctx context.Context, wd string, env []string, patterns []string, opts *GenerateOptions) ([]GenerateResult, []error) {
	...
	for i, pkg := range pkgs {
		...
		generated[i].OutputPath = filepath.Join(outDir, opts.PrefixOutputFile+"wire_gen"+opts.SuffixOutputFile+".go")
		g := newGen(pkg)
		...
		goSrc := g.doFrame(opts.Tags, opts.NoGenerateLine)
...
func (g *gen) frame(tags string) []byte {
	return g.doFrame(tags, false)
}

// frame bakes the built up source body into an unformatted Go source file.
func (g *gen) doFrame(tags string, noGenLine bool) []byte {
	if g.buf.Len() == 0 {
		return nil
	}
	...
	if !noGenLine {
		buf.WriteString("//go:generate go run github.com/github.com/noke-inc/lib_wiremann/wire/cmd/wire" + tags + "\n")
	}
